...
- 이름, 메일 주소, 연령대, 건강 특이사항, 복용약, 민감도 정보를 저장
...
- `conditions.expected_weather` 는 수동 입력 또는 Open-Meteo 기반 자동 수집 결과를 함께 저장
...
- 계획별 날씨 조사 캐시
- 날씨 질의 문자열, Open-Meteo 조회 URL, 예보 근거 발췌, 구조화 결과, 조회 시각 저장
...
- `trips/*.yaml` 원본과 분리해 저장하고 UI는 pending 분석 작업이 있을 때만 SSE 이벤트로 즉시 반영하며 재진입/재연결 시 상태 조회 API로 복구한다
